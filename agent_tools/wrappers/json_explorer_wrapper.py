#!/usr/bin/env python3
"""
Canonical execution wrapper for the `json_explorer` skill.
"""
...
def _schema_preview(value: Any, depth: int, max_depth: int, max_items: int) -> Any:
    ...
    if isinstance(value, dict):
        keys = sorted(value.keys())[:max_items]
        return {
            "type": "object",
            "key_count": len(value),
            "keys": keys,
            "children": {key: _schema_preview(value[key], depth + 1, max_depth, max_items) for key in keys},
        }
    if isinstance(value, list):
