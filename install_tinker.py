...
def merge_gitignore(
    framework_gitignore_host: Path,
    dest_gitignore: Path,
    *,
    dry_run: bool,
    report: ChangeReport,
) -> None:
    host_lines = [normalize_line(l) for l in read_text(framework_gitignore_host).splitlines()]
...
def merge_requirements(
    ...
    *,
    ...
    inv_lines = [normalize_line(l) for l in read_text(framework_reqs).splitlines()]
    ...
    if out_lines and out_lines[-1].strip():
        out_lines.append("")
    out_lines.append("# Added by Tinker installer (missing deps)")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Install Tinker into one or more projects")
    ...
    parser.add_argument(
        "--backup",
        action="store_true",
        help="When overwriting/migrating, store backups under <dest>/.tinker_backups/<timestamp>/",
    )
    parser.add_argument(
    ...
    if not args.skip_gitignore and not inv_gitignore_host.exists():
        raise SystemExit(f"Tinker gitignore.host not found: {inv_gitignore_host}")
