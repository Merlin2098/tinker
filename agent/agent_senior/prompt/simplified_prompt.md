...
1. Validate contract completeness against `user_task.schema.yaml` before acting.
2. Do not infer missing fields. Do not invent data or paths.
3. If `Phase = A_CONTRACT_VALIDATION`, return validation verdict only and stop.
4. If `Phase = B_EXECUTION` and `Validation Status != PASSED`, stop immediately.
5. Do not assume config location; use only declared `Config`.

## 3. Execution References
- Context: `agent/agent_senior/context/context.md`
- Workflow: `agent/agent_senior/workflow/workflow.md`
