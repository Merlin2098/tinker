...
- `agent_tools/wrappers/query_parquet_duckdb_wrapper.py`
...
```bash
.venv/Scripts/python.exe agent_tools/run_wrapper.py --skill query_parquet_duckdb --args-json "{\"sql_query\":\"SELECT 1 AS ok\"}"
```

Alternative with args file:

```bash
.venv/Scripts/python.exe agent_tools/run_wrapper.py --skill query_parquet_duckdb --args-file agent/agent_outputs/plans/query_parquet_duckdb.args.json
```
