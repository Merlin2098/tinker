...
- `agents/tools/wrappers/connect_duckdb_wrapper.py`
...
```bash
.venv/Scripts/python.exe agents/tools/run_wrapper.py --skill connect_duckdb --args-json "{\"database_path\":\"data/example.duckdb\",\"read_only\":false}"
```

Alternative with args file:

```bash
.venv/Scripts/python.exe agents/tools/run_wrapper.py --skill connect_duckdb --args-file agents/logic/agent_outputs/plans/connect_duckdb.args.json
```
