# Skill: context_loading_protocol (Thin Interface)

## Purpose
Loads context: treemap first, then dependencies, then task-specific
...
```bash
.venv/Scripts/python.exe agents/tools/run_wrapper.py --skill context_loading_protocol --args-json "{\"objective\":\"<goal>\"}"
```
