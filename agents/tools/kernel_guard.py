...
- the active kernel profile state file (agents/logic/agent_outputs/runtime/active_profile*.json)
- the requested profile in agents/logic/user_task.yaml (mode_profile)
...
def activate_kernel(profile: str, agent_id: Optional[str]) -> int:
    ...
    cmd = [sys.executable, str(project_root() / "agents" / "tools" / "activate_kernel.py"), "--profile", profile]
    if agent_id:
...
def main() -> int:
    ...
    parser.add_argument("--user-task", default="agents/logic/user_task.yaml", help="User task yaml path")
